...
{
	while (!(LPUART7_STAT & LPUART_STAT_TDRE)) ; // wait
	LPUART7_DATA = c;
}

FLASHMEM void printf_debug_init(void)
{
        CCM_CCGR5 |= CCM_CCGR5_LPUART7(CCM_CCGR_ON); // turn on Serial7
        IOMUXC_SW_MUX_CTL_PAD_GPIO_EMC_31 = 2; // Arduino pin 29
        LPUART7_BAUD = LPUART_BAUD_OSR(25) | LPUART_BAUD_SBR(8); // ~115200 baud
        LPUART7_CTRL = LPUART_CTRL_TE;
}
